...
class SourceQueueProxyServiceImplTest {
    ...
    @Test
    void maskSensitiveData_shouldMaskDiscoveredAddressAsCompleteObject() throws Exception {
        // Arrange
        String jsonWithFullAddress = "{\"payload\":{\"analogMail\":{\"requestId\":\"" + REQUEST_ID +
                "\",\"discoveredAddress\":{" +
                "\"name\":\"Mario Rossi\"," +
                "\"nameRow2\":\"c/o Ufficio\"," +
                "\"address\":\"Via Roma 123\"," +
                "\"addressRow2\":\"Interno 4\"," +
                "\"cap\":\"20100\"," +
                "\"city\":\"Milano\"," +
                "\"city2\":\"MI\"," +
                "\"pr\":\"MI\"," +
        ...
        when(objectMapper.writeValueAsString(message)).thenReturn(jsonWithFullAddress);

        // Act
        String result = invokeMaskSensitiveData(message);

        // Assert
        assertNotNull(result);
        assertFalse(result.contains("Mario Rossi"));
        assertFalse(result.contains("Via Roma 123"));
        assertFalse(result.contains("Milano"));
        assertTrue(result.contains("\"discoveredAddress\":\"***\""));
        verify(objectMapper, times(1)).writeValueAsString(message);
    }

    @Test
    void maskSensitiveData_shouldMaskDiscoveredAddressAsString() throws Exception {
        // Arrange
        String jsonWithStringAddress = "{\"payload\":{\"analogMail\":{\"requestId\":\"" + REQUEST_ID +
                "\",\"discoveredAddress\":\"Via Roma 123, Milano\"}}}";

        when(objectMapper.writeValueAsString(message)).thenReturn(jsonWithStringAddress);
        ...
        // Assert
        assertNotNull(result);
        assertFalse(result.contains("Via Roma 123, Milano"));
        assertTrue(result.contains("\"discoveredAddress\":\"***\""));
        verify(objectMapper, times(1)).writeValueAsString(message);
    }

    @Test
    void maskSensitiveData_withoutDiscoveredAddress_shouldReturnUnmodifiedJson() throws Exception {
        // Arrange
        String jsonWithoutAddress = "{\"payload\":{\"analogMail\":{\"requestId\":\"" + REQUEST_ID +
                "\",\"statusCode\":\"OK\",\"statusDescription\":\"RECAG012\"}}}";

        when(objectMapper.writeValueAsString(message)).thenReturn(jsonWithoutAddress);
        ...
        // Assert
        assertNotNull(result);
        assertEquals(jsonWithoutAddress, result);
        assertTrue(result.contains("\"requestId\":\"" + REQUEST_ID + "\""));
        assertTrue(result.contains("\"statusCode\":\"OK\""));
        assertFalse(result.contains("discoveredAddress"));
        verify(objectMapper, times(1)).writeValueAsString(message);
    }

    @Test
    void maskSensitiveData_withNullDiscoveredAddress_shouldMaskNullValue() throws Exception {
        // Arrange
        String jsonWithNullAddress = "{\"payload\":{\"analogMail\":{\"requestId\":\"" + REQUEST_ID +
                "\",\"discoveredAddress\":null}}}";

        when(objectMapper.writeValueAsString(message)).thenReturn(jsonWithNullAddress);
        ...
        // Assert
        assertNotNull(result);
        // Il regex dovrebbe lasciare null come null o mascherarlo
        assertTrue(result.contains("discoveredAddress"));
        verify(objectMapper, times(1)).writeValueAsString(message);
    }

    /**
     * Metodo helper che invoca il metodo privato maskSensitiveData usando reflection
     */
    private String invokeMaskSensitiveData(Object obj) throws Exception {
        var method = SourceQueueProxyServiceImpl.class.getDeclaredMethod("maskSensitiveData", Object.class);
        method.setAccessible(true);
        return (String) method.invoke(service, obj);
    }
}