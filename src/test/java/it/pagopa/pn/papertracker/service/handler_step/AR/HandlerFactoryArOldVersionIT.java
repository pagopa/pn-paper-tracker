...
public class HandlerFactoryArOldVersionIT extends BaseTest.WithLocalStack {
    ...
    void testARSequence(TestSequenceAREnum seq) {
        ...
        paperTrackingsDAO.putIfAbsent(getPaperTrackingsOldVersion(requestId, ProductType.AR)).block();
        ...
        if (seq.equals(FAIL_CON996_PCRETRY_FURTO_AR)) {
            paperTrackingsDAO.putIfAbsent(getPaperTrackingsOldVersion(r1.getRequestId(), ProductType.AR)).block();
            paperTrackingsDAO.putIfAbsent(getPaperTrackingsOldVersion(r2.getRequestId(), ProductType.AR)).block();
            when(paperChannelClient.getPcRetry(any(), eq(true)))
                    .thenReturn(Mono.just(r1));
